...
*/
    ...
    {
       $lesson= DB::table('courses_lessons')->where('lesson_id',$lessonId)->get();
        if(count($lesson)>0){
        // query del by id
    ...
    public function getLessonsByCourseId($CourseId=0)
    {
        ...
        $lesson =DB::table('courses_lessons')
            ...
            ->select('courses.course_name','courses_lessons.lesson_title','courses_lessons.lesson_description','courses_lessons.lesson_video','users.name')
